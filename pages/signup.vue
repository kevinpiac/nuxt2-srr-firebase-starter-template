...
app-container
  div(class="login-page__modal")
    span(class="login-page__modal-title") {{ $t('signupPage.title')}}
    a-form(
      ...
      template(v-if="form = form")
        ...
        a-form-item(fieldDecoratorId="password" :fieldDecoratorOptions="{}")
          ...
          a-button(type="primary" htmlType="submit") {{ $t('signupPage.basicSignUpAction')}}
  ...
  methods: {
    ...
    async handleSubmit(e) {
      ...
      try {
        ...
        this.$router.push(this.localePath('authenticated'));
      } catch {
        // handle error
      }
    },
